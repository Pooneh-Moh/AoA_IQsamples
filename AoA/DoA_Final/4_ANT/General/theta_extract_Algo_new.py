...
ant_ref = []
for i in range(17):
    ant_ref.append(complex(new_data['i'][i], new_data['q'][i]))
...
for i in range(len(ant_1)):
    multi1 = ant_1[i] * np.conj(ant_2[i])
    multi2 = ant_2[i] * np.conj(ant_3[i])
    multi3 = ant_3[i] * np.conj(ant_4[i])
    p1.append(np.arctan2(multi1.real, multi1.imag))
    p2.append(np.arctan2(multi2.real, multi2.imag))
    p3.append(np.arctan2(multi3.real, multi3.imag))

ant_final.append(ant_1)
ant_final.append(ant_2)
ant_final.append(ant_3)
ant_final.append(ant_4)
# print('the angles are',(np.multiply(p1, 1/6*np.pi)))
ant_1_ang = np.multiply(p1, 3/2*np.pi)
ant_2_ang = np.multiply(p2, 3/2*np.pi)
ant_3_ang = np.multiply(p3, 3/2*np.pi)
ant_4_ang = np.multiply(p4, 3/2*np.pi)
