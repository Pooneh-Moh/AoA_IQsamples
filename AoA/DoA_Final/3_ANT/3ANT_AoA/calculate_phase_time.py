...
new_data = pd.read_csv('info_dec_AoA_3ANT.csv')
...
for n in range(N):
    for k in range(0 + n*288, 288*(n+1), 4):
        info_per_ant.append({"pkt": new_data['pkt'][k], "channel": new_data['Channel'][k], "phi_ant1": new_data['phi_info'][k + 3],
                             "phi_ant2": new_data['phi_info'][k], "phi_ant3": new_data['phi_info'][k + 1],
                             ...
                             "meg_ant3": new_data['mag_info'][k + 1]})
result = pd.DataFrame(info_per_ant)
result.to_csv('info_per_ant_AoA_3ANT.csv', index=False)